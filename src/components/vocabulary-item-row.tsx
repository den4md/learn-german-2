...
export function VocabularyItemRow({ item, onChangeFavouriteStatus, onChangeWordState, onEditVocabularyItem }: VocabularyItemRowProps) {
  ...
  return <li className="grid gap-4 px-6 py-5 sm:px-8 lg:grid-cols-[minmax(15rem,1fr)_auto] lg:items-center">
    <div>
      <div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-slate-950">{getGermanHeadword(item)}</p><span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-700">{t(wordStateMessageKeys[item.wordState])}</span></div>
    ...
    <div className="flex flex-wrap items-center gap-2 lg:justify-end">
      ...
      <PopupMenu menuAriaLabel={t('moreActions')} menuClassName="absolute right-0 z-10 mt-2 grid w-52 gap-1 rounded-xl border border-slate-200 bg-white p-2 shadow-lg shadow-slate-300/40" onClose={() => setMenuPage('actions')} triggerAriaLabel={t('moreActions')} triggerClassName="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 active:translate-y-px focus:outline-none focus:ring-4 focus:ring-blue-100" triggerContent={t('moreActions')}>

...
function MoreActions({ onChangeWordState, onEdit }: { onChangeWordState(wordState: typeof wordStates[keyof typeof wordStates]): void; onEdit(): void }) {
  ...
  return <details className="group relative" onClick={(event) => event.stopPropagation()}><summary aria-label={t('moreActions')} className="ml-auto flex w-fit cursor-pointer list-none rounded-lg border border-slate-300 bg-white px-3 py-2 text-lg font-bold leading-none text-slate-700 marker:content-none active:translate-y-px focus:outline-none focus:ring-4 focus:ring-blue-100"><span aria-hidden="true">•••</span></summary><section className="absolute right-0 z-10 mt-2 w-64 rounded-xl border border-slate-200 bg-white p-3 shadow-lg shadow-slate-300/40" aria-label={t('moreActions')}><p className="px-1 pb-2 text-sm font-bold text-slate-950">{t('changeWordState')}</p><div className="grid gap-1">{actions.map((action) => <button className="rounded-lg px-3 py-2 text-left text-sm font-semibold text-slate-700 hover:bg-slate-50 active:translate-y-px focus:outline-none focus:ring-4 focus:ring-blue-100" key={action.value} type="button" onClick={() => onChangeWordState(action.value)}>{action.label}</button>)}<button className="rounded-lg px-3 py-2 text-left text-sm font-semibold text-slate-700 hover:bg-slate-50 active:translate-y-px focus:outline-none focus:ring-4 focus:ring-blue-100" type="button" onClick={onEdit}>{t('edit')}</button></div></section></details>
}

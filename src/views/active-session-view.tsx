...
export function ActiveSessionView({ learningData, onShowEntry, onShowCandidate, onRevealEntry, onAssessEntry, onSelectNextCandidatePage }: ActiveSessionViewProps) {
  ...
  const activeEntryIndex = activeSession?.entries.findIndex((entry) => entry.selfAssessment === undefined) ?? -1
  ...
  const completedEntryCount = activeSession.entries.filter((entry) => entry.selfAssessment !== undefined).length
  ...
  return (
    <section className="mx-auto max-w-3xl">
      ...
      {vocabularyItem === undefined ? <p className="mt-8 rounded-2xl border border-slate-200 bg-white px-6 py-16 text-center text-slate-600">{t('loadingCard')}</p> : activeEntry === undefined ? null : <Flashcard activeEntryIndex={activeEntryIndex} isRevealed={activeEntry.revealedAt !== undefined} sessionType={activeSession.type} settings={activeSession.toData().settings} vocabularyItem={vocabularyItem} onAssessEntry={onAssessEntry} onRevealEntry={onRevealEntry} />}
    </section>
  )
}

function Flashcard({ activeEntryIndex, isRevealed, sessionType, settings, vocabularyItem, onAssessEntry, onRevealEntry }: { activeEntryIndex: number; isRevealed: boolean; sessionType: string; settings: SessionSettingsData; vocabularyItem: ResolvedVocabularyItemData; onRevealEntry(entryIndex: number): void; onAssessEntry(entryIndex: number, selfAssessment: SelfAssessment): void }) {
  const { t } = useInterfaceLanguage()
